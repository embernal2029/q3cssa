...
let ownedRods=[]
const rods=[
  { name:"Carbon Rod", cost:150, changes:{Common:0.8, Uncommon:1.14, Rare:1.04, Legendary:1.02}, sprite:"sprites/rod1.png" },
  { name:"Steel Rod", cost:350, changes:{Common:0.1, Uncommon:0.5, Rare:0.25, Legendary:0.15}, sprite:"sprites/rod2.png" },
  { name:"Lava Rod", cost:500, changes:{Common:0, Uncommon:0.2, Rare:0.5, Legendary:0.3}, sprite:"sprites/rod3.png" }
]
...
const fishData = [
  ...
  { name:"Swordfish", rarity:"Rare", sprite:"sprites/swordfish.png", speed:3.4, points:50, chance:5, catchRate:0.55 },
...
function getRandomFish(){
  ...
  for(const f of fishData){if(roll<f.chance) return f; roll-=f.chance}
  return fishData[0]
}
...
function renderShop(){
  ...
  rods.forEach((rod,index)=>{
    ...
    const prevOwned = index===0 || ownedRods.includes(index-1)
    if(score<rod.cost || !prevOwned || ownedRods.includes(index)) rodDiv.classList.add("disabled")
    rodDiv.innerHTML=`<img src="${rod.sprite}"><div>${rod.name} - ${rod.cost} pts</div>`
    if(!rodDiv.classList.contains("disabled")) rodDiv.addEventListener("click",()=>buyRod(index))
...
function checkOverlap(){
  ...
  const fishTop=fishY, fishBottom=fishY+fish.clientHeight
...
muteCheckbox.addEventListener("change",()=>{bgmTracks.forEach(t=>t.muted=muteCheckbox.checked);if(!muteCheckbox.checked) selectedBgm.play()})
